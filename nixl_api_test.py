...
def test(
    ...
    logging.info(
        f"Each iteration transfers {args.blocks_per_xfer} blocks ({data_per_iteration / 1e6:.2f} MB)"
    )
    ...
    for i, ((start, end), latency_ms) in enumerate(zip(iteration_history, latencies)):
        ...
        logging.info(
            f"Iteration {i}: block_ids range is {start} to {end - 1}, Latency: {latency_ms:.3f} ms"
        )
        if is_warmup:
        ...
        rx_block_hashes = [
            compute_block_hash(local_kv_cache, sender_meta.block_len, i)
            for i in range(start, end)
        ]
        logging.info(
            f"received block is local_kv_cache[{start_idx}:{end_idx}]={local_kv_cache[start_idx:end_idx]}"
        )
        ...
        if rx_block_hashes != sender_block_hashes:
            ...
            logging.info(
                f"Data integrity verified for {args.blocks_per_xfer} transferred blocks for iteration {i}"
            )
...
def read_blocks(
    ...
    return (t1 - t0) / 1e6, len(
        local_ids
    ) * sender_meta.block_len  # Return latency in ms
...
def sender_process(args: argparse.Namespace, zmq_host: str = "127.0.0.1"):
    ...
    try:
        ...
        num_heads = 1 if args.mla else args.num_heads
        block_len = num_heads * args.head_size * args.block_size * dtype.itemsize
        if not args.mla:
            block_len *= 2
        ...
        if hasattr(kv_cache, "storage"):
            ...
            print(
                f"Storage device: {storage.device if hasattr(storage, 'device') else 'Unknown'}"
            )
        ...
        logging.info(
            f"Computing block hashes for data integrity verification...{kv_cache=}"
        )
        block_hashes = [
        ...
        reg_descs = agent.get_reg_descs(
            [
                (
                    get_data_ptr(kv_cache, args.device_type),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark script for nixl._api performance."
    )
    ...
    parser.add_argument("--mla", action="store_true", help="MLA KVCache")
    parser.add_argument("--blocks-per-xfer", type=int, default=128)
    parser.add_argument("--num-iterations", type=int, default=100)
    parser.add_argument(
        "--nixl-memory-type", type=str, default="DRAM", choices=["DRAM", "VRAM"]
    )
    parser.add_argument(
        "--device-type", type=str, default="cpu", choices=["cpu", "cuda", "hpu"]
    )
    ...
    if args.role == "sender":
        ...
        sender = multiprocessing.Process(
            target=sender_process, args=(args, args.zmq_host), name="Sender"
        )
        receiver = multiprocessing.Process(
            target=receiver_process, args=(args, args.zmq_host), name="Receiver"
        )
