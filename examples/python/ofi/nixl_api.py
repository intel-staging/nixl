...
def read_blocks(block_ids: Iterator[int], agent: NixlAgent,
    ...
    while agent.check_xfer_state(xfer_handle) != "DONE":
        check_count += 1
        if check_count % 10000 == 0:  # Log every 10k checks
            state = agent.check_xfer_state(xfer_handle)
            logging.info(f"Transfer still in progress: state={state}, checks={check_count}")
        time.sleep(0.00001)
...
def summary(latencies: List[float], sender_meta: NixlAgentMetadata, args: argparse.Namespace, total_data_transferred: int, sustained_throughput_gbps: float, total_elapsed_seconds: float, agent: NixlAgent):
    ...
    print(f"   Std Dev:    {stats['std']:8.3f} ms")
    print(f"   Min:        {stats['min']:8.3f} ms")
    print(f"   25%:        {stats['25%']:8.3f} ms")
    print(f"   50% (Med):  {stats['50%']:8.3f} ms")
    print(f"   75%:        {stats['75%']:8.3f} ms")
...
def sender_process(args: argparse.Namespace, zmq_host: str = "127.0.0.1"):
    ...
    try:
        ...
        reg_ret = agent.register_memory(reg_descs, backends=[args.nixl_backend])
...
def receiver_process(args: argparse.Namespace, zmq_host: str = "127.0.0.1"):
    ...
    try:
        ...
        logging.info(f"Sender metadata details:")
        logging.info(f"  engine_id: {sender_meta.engine_id}")
        logging.info(f"  agent_metadata: {len(sender_meta.agent_metadata)} bytes")
        logging.info(f"  kv_caches_base_addr: {sender_meta.kv_caches_base_addr}")
        logging.info(f"  num_blocks: {sender_meta.num_blocks}")
        logging.info(f"  block_len: {sender_meta.block_len}")
        logging.info(f"  block_hashes: {sender_meta.block_hashes[:3]}{'...' if len(sender_meta.block_hashes) > 3 else ''} ({len(sender_meta.block_hashes)} total)")
        ...
        # Calculate total data transferred: iterations * blocks_per_xfer * block_len
